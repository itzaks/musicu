var m = {
	...
	init : function() {
		...
		$('#music_search input.searchval').focus();
	...
	add_fixed : function() {
		...
		if (!msie6) {
			$(window).scroll(function (event) {
				...
				if (y >= 67) {
				$('header').addClass('fixed').animate({'top' : -67}, 100);
			} else {
	...
	binds : function() {
		...
		$(".music_search input").focus(function() {
			$(this).select();
		});
		...
		$("li.artist_album", m.$a_albums).live("click", function(ev) {
			...
			$(this).addClass("open");
			m.view_tracks(id, $("var.artist", this).text(), $("div.tracks", this), $("var.type", this).text());
		});
			...
			if(results.status == true) {
				...
				for(var i = 0, il = searchresults.length; i<il; i ++) {
					var element = 	'<li class="result">' + 
									'<span class="name ' + searchresults[i].type + '">' + searchresults[i].title + '</span>' + 
	...
	render_artist : function(result) {
		...
		for(var i = 0, il = releases.length; i<il; i ++) {
			if(releases[i].role == "Main") {
				...
				if(name.length > 49) {
					name = name.slice(0, name.length-49) + "...";
				}
	...
	play_track : function(el) {
		...
		artist = artist.replace(/\([0-9][0-9]\)/g, "");
		...
		$.getJSON(query, function(json) {
			if(undefined != json.feed.entry) {
				...
				m.player.load(url);
			} else {console.log("couldn't find track..")}
		});
	},
